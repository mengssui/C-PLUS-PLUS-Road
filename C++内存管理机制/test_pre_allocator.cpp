 ...
 * @version: 0.1
 * @Author: Mengbw
 * @Date: 2021-06-10 22:24:48
 * @LastEditors: Mengbw
 * @LastEditTime: 2021-06-15 23:08:58
 */
#include <cstddef>
#include <iostream>
...
void* Screen::operator new(size_t size)
{
  ...
  p = freeStore;                // 返回这个小块内存
  freeStore = freeStore->next;  // 函数指向下一个可用的内存块
  return p;
}
...
Screen* p[N];	
   	...
   	for (int i=0; i< N; ++i)
   	     p[i] = new Screen(i);  //运行Screen重载的operator new, 经过验证，仅差16; 使用全局则间隔32
