...
void LogFile::append_unlocked(const char* logline, int len)
{
  ...
  {
    if (count_ > kCheckTimeRoll_) //检测是否需要滚动日志,避免每次输出都去检查
    {
      count_ = 0;
      ...
      else if (now - lastFlush_ > flushInterval_) //从缓冲写入文件中
      {
        lastFlush_ = now;
        file_->flush();
      }
...
void LogFile::rollFile()
{
  ...
  if (now > lastRoll_) //为什么要比较这句话呢？
  {
    lastRoll_ = now;
    lastFlush_ = now;
    startOfPeriod_ = start;
    file_.reset(new File(filename)); //智能指针指向新的内容，之前的释放
  }
}
...
string LogFile::getLogFileName(const string& basename, time_t* now)
{
  ...
  struct tm tm;
  *now = time(NULL);
  gmtime_r(now, &tm); // FIXME: localtime_r ? //线程安全的
